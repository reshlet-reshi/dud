...
Citation note: this version replaces the deep-research UI citation tokens with normal markdown links. Citations to the uploaded project handoff point to the companion file [`research-prompt-0.md`](research-prompt-0.md); external citations point directly to their public source URLs.

## 1. Executive summary

I treated the attached handoff as the governing brief for repo identity, settled constraints, owner priorities, current `dud-sh` subset, the proposed layout, and the `patch-elf`-first bootstrap ladder. This report does not try to reopen choices the handoff already marks as decided; it focuses on the remaining policy decisions that matter most for a future `AGENTS.md`. Source basis: [`research-prompt-0.md`](research-prompt-0.md).
...
Source: [`research-prompt-0.md`](research-prompt-0.md).
...
**Why it fits:** it best matches the owner’s preference for correctness, conformance testing, hidden dependency avoidance, and future non-shell hosts. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
**Why it conflicts:** it weakens the stated preference to avoid hidden host dependencies. If the project really wants this mode, AGENTS must say so bluntly: this is not “plain POSIX”; it is “common GNU-ish host profile.” Source: [`research-prompt-0.md`](research-prompt-0.md).
...
**Why it fits:** it best serves the owner priorities in the handoff: technical correctness, fast iteration through clean tests, simplicity, learning, maintainability, and low dependency risk. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
`patch-elf` should be explicitly named as the first native artifact, and `patch-elf-modular` as the first modular artifact. Labels and relocations should be explicitly out of scope for stage 0. The bootstrap graph belongs in `docs/` and should be summarized in root/subproject READMEs; AGENTS should contain only the operational rules and pointer. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
`exec`, FD redirection, `$0`, `$#`, `shift`, pipelines, functions, variables, loops, `exit`, and `return` should be **reserved future / not current subset**. Do not merely omit them, because omission invites agents to “helpfully” introduce them. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
Invalid-script conformance should specify accept/reject, byte output, and exit status where practical, but should not require exact stderr text. That follows the handoff’s conformance rule. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
Every byte-emitting `printf` should be a logical unit: one instruction, one header field bundle, one marker record, or one small conceptual byte word. Do not explode everything to one byte per line unless that is genuinely clearer. Leaf instruction files may emit a small logical bundle when clearer than single-byte fragments. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
The suite should be organized as conformance cases that later host implementations can run. `/bin/sh` can serve as a compatibility oracle only for valid shared-subset scripts, not for invalid scripts where shell diagnostics vary. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
Agents should browse/re-check primary sources whenever touching instruction encodings, syscall numbers, ELF headers, POSIX portability, or licensing. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
The proposed marker byte pattern `90 6a ID 8d 64 24 04` should be treated as **provisional**, not settled. Standardize semantics now: marker records must be executable-safe or trivially skippable, documented, and test-covered. Defer exact byte ABI until the first implementation and stripper/consumer logic exist. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
Do not optimize for conventional assembler design too early. `dud-asm` and `dud-cc` should remain long-term context only. Source: [`research-prompt-0.md`](research-prompt-0.md).
...
- [`research-prompt-0.md`](research-prompt-0.md) — uploaded project handoff and governing brief.

# proto/ctok/test context doc

## Context policy

- Read this before changing `proto/ctok/test/main.sh`, coverage reporting, or this
  directory layout.
...
- `proto/ctok/runme.sh` is explicitly mobile: callers pass
...
- No-coverage repo-local example:
  `./proto/ctok/runme.sh --ctok-dir ./proto/ctok --cc ./.dud/x86_64-linux-musl-native/bin/x86_64-linux-musl-gcc --expect ./.dud/expect --out-dir ./.dud/experiments/ctok --coverage no`.
...
- `proto/ctok/test/main.sh` always runs behavior cases against the
...
- `proto/ctok/test/coverage-report.sh` owns gcov stdout parsing, `main.c.gcov`
  parsing, normalized model validation, parser-integrity checks, and report
  formatting, using explicit `--gcov`, `--awk`, and `--sed` paths.
- `proto/ctok/test/coverage-report-test.sh` owns fixture-driven reporter tests
...
- Split coverage reporting out of `proto/ctok/test/main.sh` into
  `proto/ctok/test/coverage-report.sh` and `proto/ctok/test/coverage-report-test.sh`.
...
- Add a new `### Turn N` whenever work resumes on `proto/ctok/test/main.sh`,
...
- Established this context process, moved tests/sources under `src/foo/test`
  ...
  fixtures, and ported exact-output helpers to `proto/expect/main.c`.
...
- User asked to move expect, ctok, and dud-sh into separate top-level
  directories; expect later moved to `proto/expect`.

### Turn 21

- User asked to move ctok and dud-sh under `proto/` and unhook them
  from root `./runme.sh`.

...
This bootstrap runme builds only the static x86_64 startup set:
...
- The subshell keeps `cd "$musl_src"` local to this runme step.
...
After building these objects under `$musl_obj`, `proto/musl-tcc/runme.sh` copies them
to `$musl_lib`:

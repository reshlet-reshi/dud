...
usage() {
    printf '%s\n' \
        'usage: proto/musl-tcc/runme.sh --musl-tcc-dir DIR --bootstrap-cc CC --install-dir DIR --work-dir DIR --tar TAR --patch PATCH --sed SED --find FIND --sort SORT --grep GREP --cmp CMP --sha256sum SHA256SUM --chmod CHMOD --cp CP --rm RM --mv MV --mkdir MKDIR --dd DD --tr TR --dirname DIRNAME' \
        >&2
}

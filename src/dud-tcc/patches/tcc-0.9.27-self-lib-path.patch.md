...
under `.init/bootstrap-tcc`. TCC already has a `{B}` path substitution mechanism, but
...
```text
.init/bootstrap-tcc/tcc
.init/bootstrap-tcc/include
.init/bootstrap-tcc/lib
.init/bootstrap-tcc/libtcc1.a
```
...
`.init/bootstrap-tcc/lib`. The compiler could still be used with an explicit `-B`, but
...
```text
install: <repo>/.init/bootstrap-tcc
include:
  <repo>/.init/bootstrap-tcc/include
libraries:
  <repo>/.init/bootstrap-tcc/lib
  <repo>/.init/bootstrap-tcc
libtcc1:
  <repo>/.init/bootstrap-tcc/libtcc1.a
crt:
  <repo>/.init/bootstrap-tcc/lib
elfinterp:
  -
...
- Moving only `.init/bootstrap-tcc/tcc` without its sibling `include/`, `lib/`, and
...
Linux x86_64 bootstrap and installs a self-contained tree under `.init/bootstrap-tcc`.

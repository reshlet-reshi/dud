...
```text
.init/musl-tcc/tcc
.init/musl-tcc/include
.init/musl-tcc/lib
.init/musl-tcc/libtcc1.a
```
...
`.init/musl-tcc/lib`. The compiler could still be used with an explicit `-B`, but
...
```text
install: <repo>/.init/musl-tcc
include:
  <repo>/.init/musl-tcc/include
libraries:
  <repo>/.init/musl-tcc/lib
  <repo>/.init/musl-tcc
libtcc1:
  <repo>/.init/musl-tcc/libtcc1.a
crt:
  <repo>/.init/musl-tcc/lib
elfinterp:
  -
...
- Moving only `.init/musl-tcc/tcc` without its sibling `include/`, `lib/`, and
...
Linux x86_64 bootstrap and installs a self-contained tree under `.init/musl-tcc`.
